...
export default function BuySuccessModal({ open, onOk }: Props) {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-[90] grid place-items-center px-4 bg-black/30">
      <div className="w-full max-w-[360px] bg-white rounded-[24px] px-8 py-10 text-center shadow-xl">
        ...
        <p className="mt-2 text-[13px] text-slate-500 leading-6">
          Your crypto will be delivered to your wallet shortly.
        </p>
        <button
          type="button"
          onClick={onOk}
          className="mt-6 h-[44px] px-10 rounded-[12px] bg-emerald-600 text-white font-semibold text-[14px] cursor-pointer hover:brightness-110 transition"
        >
          Ok
        </button>
      </div>
